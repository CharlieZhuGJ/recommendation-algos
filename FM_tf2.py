...
class FM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # Input data.
            self.train_features = tf.compat.v1.placeholder(tf.int32, shape=[None, None])  # None * features_M
            self.train_labels = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])  # None * 1
            self.dropout_keep = tf.compat.v1.placeholder(tf.float32)
            ...
            nonzero_embeddings = tf.nn.embedding_lookup(params=self.weights['feature_embeddings'],
                                                        ids=self.train_features)
            self.summed_features_emb = tf.reduce_sum(input_tensor=nonzero_embeddings, axis=1)  # None * K
            # get the element-multiplication
            self.summed_features_emb_square = tf.square(self.summed_features_emb)  # None * K

            # _________ square_sum part _____________
            self.squared_features_emb = tf.square(nonzero_embeddings)
            self.squared_sum_features_emb = tf.reduce_sum(input_tensor=self.squared_features_emb, axis=1)  # None * K

            # ________ FM __________
            self.FM = 0.5 * tf.sub(self.summed_features_emb_square, self.squared_sum_features_emb)  # None * K
            if self.batch_norm:
                self.FM = self.batch_norm_layer(self.FM, train_phase=self.train_phase, scope_bn='bn_fm')
            self.FM = tf.nn.dropout(self.FM, 1 - (self.dropout_keep))  # dropout at the FM layer

            # _________out _________
            Bilinear = tf.reduce_sum(input_tensor=self.FM, axis=1, keepdims=True)  # None * 1
            self.Feature_bias = tf.reduce_sum(
                input_tensor=tf.nn.embedding_lookup(params=self.weights['feature_bias'], ids=self.train_features),
                axis=1)  # None * 1
            ...
            if self.loss_type == 'square_loss':
                if self.lamda_bilinear > 0:
                    self.loss = tf.nn.l2_loss(tf.sub(self.train_labels, self.out)) + tf.keras.regularizers.l2(
                        0.5 * (self.lamda_bilinear))(self.weights['feature_embeddings'])  # regulizer
                else:
                ...
                if self.lambda_bilinear > 0:
                    self.loss = tf.contrib.losses.log_loss(self.out, self.train_labels, weight=1.0, epsilon=1e-07,
                                                           scope=None) + tf.keras.regularizers.l2(
                        0.5 * (self.lamda_bilinear))(self.weights['feature_embeddings'])  # regulizer
                else:
                    self.loss = tf.contrib.losses.log_loss(self.out, self.train_labels, weight=1.0, epsilon=1e-07,
                                                           scope=None)
            ...
            if self.optimizer_type == 'AdamOptimizer':
                ...
                self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=self.learning_rate).minimize(
                    self.loss)
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0:
            weight_saver = tf.compat.v1.train.import_meta_graph(self.save_file + '.meta')
            ...
            all_weights['feature_embeddings'] = tf.Variable(
                tf.random.normal([self.features_M, self.hidden_factor], 0.0, 0.01),
                name='feature_embeddings')  # features_M * K
            all_weights['feature_bias'] = tf.Variable(
                tf.random.uniform([self.features_M, 1], 0.0, 0.0), name='feature_bias')  # features_M * 1
